...
它不是稍后读工具，不是书签管理器，不是长期浏览历史工具，也不是资料库。它默认显示桌面 Chrome 当前打开的页面状态，并允许回看最近三天内的短历史快照。
...
系统优先展示最新的桌面 Chrome tab snapshot。Worker + D1 后端同时保存最近三天内发生变化的 snapshot 历史，超过三天的历史应被删除。

如果一个标签页在桌面端关闭，下一次同步后手机端也不再显示它。

如果需要回看刚刚关闭或切换前的状态，手机端可以在“最近三天”时间线上选择历史 snapshot。
...
- 上传时覆盖当前用户、当前设备的最新 snapshot，并在内容变化时追加一条三天内历史。
...
- Worker 后端下读取最近三天内的历史 snapshot 列表。
...
- 手机页面能切换最近三天内的历史 snapshot。
- 多台桌面设备同步后，手机端能切换设备，历史时间线按当前设备过滤。
- 搜索标题关键词能过滤列表。
- 搜索 URL 或域名能过滤列表。
- 点击结果能打开原 URL。
- 网络失败时有明确状态，不清空已有列表。
...
- 手机网页可以用同一邮箱登录。
- 扩展能上传当前所有普通窗口 tabs。
- Worker + D1 能保存最新 snapshot 和最近三天内的历史 snapshot。
- 手机网页能展示最新 snapshot。
- 手机网页能切换最近三天内的历史 snapshot。
