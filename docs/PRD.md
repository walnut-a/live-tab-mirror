...
产品形态：桌面 Chrome 扩展 + 手机网页/PWA + Supabase 后端  
...
它不是稍后读工具，不是书签管理器，不是浏览历史工具，也不是资料库。它只显示桌面 Chrome 当前打开的页面状态。
...
系统只保存最新的桌面 Chrome tab snapshot。每次同步覆盖上一份数据。

如果一个标签页在桌面端关闭，下一次同步后手机端也不再显示它。
...
- 登录入口：输入邮箱，接收 Supabase Email OTP 验证码。
...
- 登录入口：同样使用 `zhaowork74@gmail.com` + Email OTP。
- 最近同步时间。
- 搜索框，支持标题和 URL 本地搜索。
...
- 系统使用 Supabase Auth。
- 登录方式采用 Email OTP，不优先使用 Magic Link。
...
- 输入 `zhaowork74@gmail.com` 能收到验证码并登录。
...
- 客户端登录时使用 Email OTP，并设置不自动创建新用户。
- Supabase Auth 邮件模板使用 OTP code，而不是只依赖 Magic Link。
...
OTP 比 Magic Link 更适合插件，但每台设备首次登录要输入验证码。应对方式：
